...
export function DonorCommunicationQuickAccess({ unreadCount = 0 }: DonorCommunicationProps) {
  const router = useRouter();

  return (
    <Button
      onClick={() => router.push('/donor/messages')}
      variant="outline"
      size="sm"
...
// Floating Action Button for Donor Communication
export function DonorCommunicationFloatingButton({ unreadCount = 0 }: DonorCommunicationProps) {
  const router = useRouter();
