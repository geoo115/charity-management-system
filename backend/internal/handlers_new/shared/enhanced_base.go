...
func (h *EnhancedBaseHandler) ValidateAndBind(c *gin.Context, obj interface{}) error {
	...
	if err := c.ShouldBindJSON(obj); err != nil {
		h.ValidationError(c, "INVALID_JSON", "Invalid JSON format", map[string]interface{}{
			"error": err.Error(),
		})
		return err
	}
...
func (h *EnhancedBaseHandler) Error(c *gin.Context, status int, code, message string, details map[string]interface{}) {
	errorInfo := &ErrorInfo{
		Code:    code,
		Message: message,
		Details: details,
...
func (h *EnhancedBaseHandler) ValidationError(c *gin.Context, code, message string, details map[string]interface{}) {
...
func (h *EnhancedBaseHandler) logError(c *gin.Context, status int, code, message string, details map[string]interface{}) {
	logLevel := "INFO"
	if status >= 500 {
		logLevel = "ERROR"
	} else if status >= 400 {
